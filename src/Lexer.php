...
class Lexer implements \Twig_LexerInterface
{
    ...
    public function __construct(Environment $env, array $options = [])
    {
        ...
        $this->regexes = [
            'lex_var' => '/\s*(?:'.preg_quote($this->options['whitespace_trim'], '/').'|'.preg_quote($this->options['whitespace_line_trim'], '/').')'.preg_quote($this->options['tag_variable'][1], '/').'\s*|\s*'.preg_quote($this->options['tag_variable'][1], '/').'/A',
            'lex_block' => '/\s*(?:(?:'.preg_quote($this->options['whitespace_trim'], '/').'|'.preg_quote($this->options['whitespace_line_trim'], '/').')'.preg_quote($this->options['tag_block'][1], '/').'\s*|\s*'.preg_quote($this->options['tag_block'][1], '/').')\n?/A',
            'lex_raw_data' => '/('.preg_quote($this->options['tag_block'][0], '/').'(?:'.preg_quote($this->options['whitespace_trim'], '/').'|'.preg_quote($this->options['whitespace_line_trim'], '/').')?)\s*(?:end%s)\s*(?:(?:'.preg_quote($this->options['whitespace_trim'], '/').'|'.preg_quote($this->options['whitespace_line_trim'], '/').')'.preg_quote($this->options['tag_block'][1], '/').'\s*|\s*'.preg_quote($this->options['tag_block'][1], '/').')/s',
            'operator' => $this->getOperatorRegex(),
            'lex_comment' => '/(?:(?:'.preg_quote($this->options['whitespace_trim'], '/').'|'.preg_quote($this->options['whitespace_line_trim'], '/').')'.preg_quote($this->options['tag_comment'][1], '/').'\s*|'.preg_quote($this->options['tag_comment'][1], '/').')\n?/s',
            'lex_block_raw' => '/\s*(raw|verbatim)\s*(?:(?:'.preg_quote($this->options['whitespace_trim'], '/').'|'.preg_quote($this->options['whitespace_line_trim'], '/').')'.preg_quote($this->options['tag_block'][1], '/').'\s*|\s*'.preg_quote($this->options['tag_block'][1], '/').')/As',
    ...
    protected function lexData()
    {
        ...
        if (isset($this->positions[2][$this->position][0])) {
            if ($this->positions[2][$this->position][0] === $this->options['whitespace_line_trim'] && $pos = strrpos($text, "\n")) {
